...
public interface AlertDao extends JpaRepository<Alert,Integer> {
    ...
    @Query("SELECT a FROM Alert a WHERE DATE(a.createdDate) >= :pastDate AND a.forTravelService = true AND a.forAll = true  ORDER BY a.alertId DESC")
    List<Alert> getAllAlertsForBusOwnerForPrevious10Days(Date pastDate);
