...
public interface DiscountDao extends JpaRepository<Discount,Integer> {
    ...
    @Query("SELECT d FROM Discount d WHERE (d.route.routeId is null OR d.route.routeId = :routeId) AND (0 = :busOwnerId OR d.busOwner.busOwnerId = :busOwnerId) AND ('' = :discountDate OR (d.discountStartDate <= DATE(:discountDate)) AND d.discountExpiryDate >= DATE(:discountDate)) ")
